                ...
                <?php if ($popularArticles) : ?>
                    <?php foreach ($popularArticles as $key => $popular) : ?>
                        <div class="latest_style_1_item my-3">
                            ...
                            <figure class="alith_news_img"><a href="<?= URL_ROOT . "article/{$popular->article_id}/{$token}" ?>"><img src="<?= get_image($popular->thumbnail) ?>" alt="<?= $popular->thumbnail_caption ?>" /></a></figure>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
