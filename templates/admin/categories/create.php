...
<?php $this->setTitle($title ?? "Admin | Create Categories"); ?>
...
<div class="row g-5">
    ...
    <div class="col-12">
        ...
        <div class="row">
            <?= BootstrapForm::selectField("Category Parent", "child", old_select("section", $category["child"] ?? ''), $categoryOpts, ['class' => 'form-control'], ['class' => 'col-4 mb-3'], $errors) ?>

            <?= BootstrapForm::selectField("Section", "section", old_select("section", $category["section"] ?? ''), $sectionOpts, ['class' => 'form-control'], ['class' => 'col-4 mb-3'], $errors) ?>

            <?= BootstrapForm::selectField("Status", "status", old_select("status", $category["status"] ?? ''), $statusOpts, ['class' => 'form-control'], ['class' => 'col-4 mb-3'], $errors) ?>
        </div>
