...
<div class="row g-5">
        ...
        <div class="d-flex flex-column justify-content-center align-items-center p-2">
            <div class="card shadow my-2 p-2 w-100">
                ...
                <?php if ($upload_type === "file") : ?>
                    <a href="<?= URL_ROOT ?>admin/create-advert?ut=link" class="btn btn-outline-secondary my-2">Link Upload</a>
                <?php else : ?>
                    <a href="<?= URL_ROOT ?>admin/create-advert?ut=file" class="btn btn-outline-primary my-2">File Upload</a>
                <?php endif; ?>
            </div>
    ...
    <div class="col-md-7 col-lg-8">
        <h4 class="mb-3">Create Advert</h4>
        <div class="row g-3">
        ...
        <?= BootstrapForm::selectField("Advert Priority", "priority", old_select("priority", $advert["priority"] ?? ''), $priorityOpts, ['class' => 'form-control'], ['class' => 'col-sm-12 mb-3'], $errors) ?>

        <?= BootstrapForm::selectField("Advert Status", "status", old_select("status", $advert["status"] ?? ''), $statusOpts, ['class' => 'form-control'], ['class' => 'col-sm-12 mb-3'], $errors) ?>

        <?= BootstrapForm::submitButton("Create Advert", "btn btn-dark btn-sm mx-1 mb-2 fs-6 w-100") ?>
    </div>
</div>
